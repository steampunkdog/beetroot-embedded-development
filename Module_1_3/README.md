...
Навідміну від основного завдання, замість окремого піна для кожного світлодіоду використовується лише один пін. Цей пін подає сигнал на транзистор, що керує схемою.
Додано кнопку, що подає сигнал на пін плати, цей сигнал зчитується в коді і, в залежності від його значення - змінюється швидкість мерехтіння світлодіодів.

#### *Контроль світлодіодів одним піном*
Код почергово, з затримкою, подає високий і низький сигнали на транзистор *Q3* через пін *15*. Транзистор, *Q3*, у відкритому стані, пропускає струм на *Q2*, оскільки *D2 LED* підключено параллельно до *Q2*(логічне НЕ), струм перестає поступати на нього і він вимикається. Так само параллельно до транзистора *Q2*(логічне НЕ) підключено транзистор *Q1*, він закривається при відкритті *Q2*, через що струм починає надходити на *D3 LED*(логічне НЕ)

![img](one-pin-police-lights/misc/scheme.png)

#### *Зміна швидкості мерехтіння*
Кнопку заживлено від плати та підключено до піна *16*. При натисканні кнопка подає сигнал *HIGH* на визначений пін(*16*). Цей сигнал зчитується платою і в залежності від стану кнопки, на початку кожної ітерації основного циклу, встановлюється затримка мерехтіння
